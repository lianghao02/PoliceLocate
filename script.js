    ...
    async function copy() {
        ...
        try {
            ...
            if (navigator.clipboard && navigator.clipboard.write) {
                try {
                    // 嘗試同時寫入 (部分瀏覽器支援) - 但通常文字圖片不能混寫，或支援度差
                    // 策略：優先複製圖片 (因為圖片較難手動複製)
                    await navigator.clipboard.write([
                        new ClipboardItem({ 'image/png': blob })
                    ]);
                    alert('✅ 地圖截圖已複製！\n請先貼上圖片，再複製文字。');

                    // 提供一個後續動作讓使用者複製文字 (或自動延遲複製? 不建議，會覆蓋)
                    // 這裡簡單做：若成功複製圖片，則詢問是否要複製文字
                    if (confirm('圖片已複製。是否接著複製文字？\n(這將會覆蓋剪貼簿中的圖片，建議先去貼上圖片後再來點確定)')) {
                        await navigator.clipboard.writeText(t);
                        alert('✅ 文字已複製！');
                    }
                } catch (err) {
                    // 若寫入圖片失敗 (例如 Firefox 預設不開啟 async clipboard)
    ...
    async function share(type) {
        ...
        if (navigator.share && navigator.canShare) {
            try {
                ...
                console.log('Web Share API failed, fallback to link', e);
            }
        }
